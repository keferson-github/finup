...
export const RecentTransactions: React.FC<RecentTransactionsProps> = ({ 
  ...
  return (
    <div className="card p-0">
      <div className="px-6 pt-6 pb-4">
        <h3 className="text-lg font-semibold text-fg-default dark:text-fg-dark-default">Todas as Transações Recentes</h3>
      </div>
      <div className="p-6">
        {!data || data.length === 0 ? (
          <div className="text-center py-8">
            <CreditCard className="h-12 w-12 text-fg-muted dark:text-fg-dark-muted mx-auto mb-4" />
            <p className="text-fg-muted dark:text-fg-dark-muted">Nenhuma transação recente</p>
          </div>
        ) : (
          <div className="max-h-96 overflow-y-auto">
            <div className="space-y-2">
              {data.filter(transaction => transaction && transaction.id).map((transaction) => (
                <div key={transaction.id} className="flex items-center justify-between p-2 bg-canvas-subtle dark:bg-canvas-dark-subtle rounded-lg hover:bg-neutral-muted dark:hover:bg-neutral-dark-muted transition-colors cursor-pointer">
                  <div className="flex items-center">
                    <div 
                    ...
                    <div>
                      <div className="flex items-center">
                        <p className="font-medium text-fg-default dark:text-fg-dark-default mr-2 text-xs">
